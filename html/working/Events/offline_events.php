...
session_start();
  ...
  if(!isset($_SESSION['loggedin']) || $_SESSION['loggedin']==false || !isset($_SESSION['isOrg']) || $_SESSION['isOrg']){
    session_destroy();
  ...
  if(isset($_POST['submit'])){
    if(!empty($_POST['search'])){
      ...
      $filter = $_POST['filter'];
      if($filter === 'category')
        $result = mysqli_query($conn,"SELECT * FROM Offline_event NATURAL JOIN Organization_category WHERE $filter LIKE '%$temp%'");
      else  
...
<link href="http://netdna.bootstrapcdn.com/font-awesome/4.0.3/css/font-awesome.css" rel="stylesheet">  
 <link href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" rel="stylesheet"/>
  ...
  <form action="offline_events.php" method="POST" style="overflow-y:scroll;">
  <?php
  $user = $_SESSION['id'];
