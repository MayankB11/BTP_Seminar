...
session_start();
  ...
  else
  {
    ...
    if(isset($_POST['donate']))
    {
      $user = $_SESSION['id'];
      $am = $_POST['amount'];
      $check = mysqli_query($conn, "SELECT wallet FROM User WHERE user_id = '$user'");
      ...
      if($check['wallet'] >= $am)
      {
        ...
        if(mysqli_num_rows($result) == 0)
        {
          ...
          $ins = mysqli_query($conn, "UPDATE User SET wallet = wallet - $am  WHERE user_id = '$user'");
        }
        else 
        {
          $time_stamp = date('Y-m-d G:i:s');
          ...
          $upd = mysqli_query($conn, "UPDATE User SET wallet = wallet - $am  WHERE user_id = '$user'");
        }
